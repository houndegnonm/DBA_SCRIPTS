...
do
	...
	if [ "$status_check" = "5.7.23-log" ] 
	then
		uptime=$(mysqladmin --defaults-extra-file=$path/cconnection.cnf  -h ${hosts[index]}  version | grep -i uptime)
		uptime=${uptime:10:100}
		message=" - ${hostName[index]} DB is UP, Uptime: $uptime"
	else
		message=" - ${hostName[index]} DB is DOWN"
		((nb_up--))
	fi
	body="$body \n $message"
done
...
then
	...
	bash slack_message.sh -h https://hooks.slack.com/services/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 -c dwbi_db_monitoring -u MySQL-STATUS -i $emoji -m "$body" -T "$title"
fi
